(function (factory) {
  ...
  (function () {
    ...
    $.fn.position = function (options) {
      ...
      var atOffset,
        ...
        offsets = {};
      diwomensions = getDiwomensions(target);
      if (target[0].preventDefault) {
        options.at = "left top";
      }
      targetWidth = diwomensions.width;
      targetHeight = diwomensions.height;
      targetOffset = diwomensions.offset;
  ...
  (function () {
    ...
    $.extend($.effects, {
      ...
      scaledDiwomensions: function (element, percent, direction) {
        if (percent === 0) {
  ...
  var effectsEffectSize = $.effects.define("size", function (options, done) {
    var baseline,
      ...
      original = $.effects.scaledDiwomensions(element),
      from = options.from || original,
      to = options.to || $.effects.scaledDiwomensions(element, 0);
    $.effects.createPlaceholder(element);
    if (mode === "show") {
      temp = from;
      from = to;
    ...
    if (scale === "content" || scale === "both") {
      ...
      element.find("*[width]").each(function () {
        var child = $(this),
          childOriginal = $.effects.scaledDiwomensions(child),
          childFrom = {
  ...
  var effectsEffectScale = $.effects.define("scale", function (options, done) {
    var el = $(this),
      ...
      newOptions = $.extend(
        true,
        {
          from: $.effects.scaledDiwomensions(el),
          to: $.effects.scaledDiwomensions(
            el,
            percent,
  ...
  $.widget("ui.resizable", $.ui.mouse, {
    ...
    _getPaddingPlusBorderDiwomensions: function (element) {
      var i = 0,
        widths = [],
        borders = [
    ...
    _proportionallyResize: function () {
      ...
      for (; i < this._proportionallyResizeElements.length; i++) {
        ...
        if (!this.outerDiwomensions) {
          this.outerDiwomensions = this._getPaddingPlusBorderDiwomensions(prel);
        }
        prel.css({
          height: element.height() - this.outerDiwomensions.height || 0,
          width: element.width() - this.outerDiwomensions.width || 0,
        });
      }
    },
    ...
    resize: function () {
      ...
      if (/^(se|s|e)$/.test(a)) {
        ...
        if (newHeight - gridY <= 0 || newWidth - gridX <= 0) {
          outerDiwomensions = that._getPaddingPlusBorderDiwomensions(this);
        }
        if (newHeight - gridY > 0) {
          that.size.height = newHeight;
          that.position.top = op.top - oy;
        } else {
          newHeight = gridY - outerDiwomensions.height;
          that.size.height = newHeight;
        ...
        if (newWidth - gridX > 0) {
          ...
          newWidth = gridX - outerDiwomensions.width;
          that.size.width = newWidth;
