...
$sql = "Select * from cart_tbl,customer_tbl,category_tbl,style_tbl,add_ons_tbl,order_tbl,order_detail_tbl where cart_tbl.customer_id=customer_tbl.customer_id
		...
		AND order_tbl.customer_id=$order_id";
// echo $sql;  
...
if (isset($row1['bt_id'])) {
	...
	$qry = "select * from cart_tbl,bt_measurement_tbl where cart_tbl.bt_id=bt_measurement_tbl.bt_id";
	$test = mysqli_query($cnn, $qry);
	$result = mysqli_fetch_array($test);
	...
	/*$response .= "<tr>";
	$response .= "<th> BLOUSE MEASUREMENT </th><td>".$row1['bt_id']."</td>";
	$response .= "</tr>";*/
}
$response .= "</table>";
