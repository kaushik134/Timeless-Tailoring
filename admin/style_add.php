...
require_once('connect.php');
 ...
 if(!isset($_SESSION['admin_email']))
 {
  ...
  $qry1="select * from style_tbl";
  $result=mysqli_query($cnn,$qry1);
  $subcat=$cat=$sname=$price=$img='';
   if(!empty($_GET['up_id']))
   {
      $str1="select * from style_tbl";
      $result1=mysqli_query($cnn,$str1);
      $str="select * from style_tbl where style_id=".$_GET['up_id'];
        $result=mysqli_query($cnn,$str);
      $res=mysqli_fetch_array($result);
      $cat=$res['category_id'];
      $subcat=$res['subcategory_id'];
      $sname=$res['style_name'];
      $price=$res['style_price'];
      $img=$res['style_image'];
    }
  if(isset($_POST['sbtn']))
  { 
    ...
    if(!empty($_GET['up_id']))
    {
      ...
      $style_status=1; 
      $str="update style_tbl set category_id='".$_POST['cat']."',subcategory_id='".$_POST['subcat']."',style_name='".$_POST['sname']."',style_image='".$image."',style_price='".$_POST['price']."'where style_id=".$_GET['up_id'];
      //echo $str;die;
    ...
    //echo "hii"; die;
      $style_status=1;
      $qry2="insert into style_tbl values(NULL,'".$_POST['cat']."','".$_POST['subcat']."','".$_POST['sname']."','".$image."','".$_POST['price']."','".$style_status."') ";
    //echo $qry2; die;
      //mysqli_query($cnn,$qry2);
       $style_success=mysqli_query($cnn,$qry2);
           if($style_success)
           {
             $_SESSION['success']="Style Added Successfully...";
           }
    }
...
<!-- /index.html by,  04:41:56 GMT -->
  <head>
    ...
    <title> Add Style | <?php echo $title;?></title>
        ...
        <div class="content-header row">
          <div class="content-header-left col-12 mb-2 mt-1">
            <div class="breadcrumbs-top">
              <h5 class="content-header-title float-left pr-1 mb-0">Style</h5>
              <div class="breadcrumb-wrapper d-none d-sm-block">
                <ol class="breadcrumb p-0 mb-0 pl-1">
                  ...
                  <li class="breadcrumb-item active">Add Style
                  </li>
                </ol>
              </div>
            </div>
  ...
  <div class="row">
    <div class="col-md-12">
      ...
      <div class="form-group">
              <label class="form-label" for="basic-default-name"> subcategory</label>
              <select class="form-control" name="subcat">
        <option value=" " selected disabled>Select SubCategory</option>
        <?php 
        $qry="select * from subcategory_tbl";
        $test=mysqli_query($cnn,$qry);

        while($row=mysqli_fetch_array($test))
        { if($row['subcategory_status']==1){ ?>
        <option value="<?php echo $row['subcategory_id']; ?>" <?php if($row['subcategory_id']==$subcat){ echo "selected"; } ?>> <?php echo $row['subcategory_name']; ?> </option>
        <?php }} ?>
        </select>
            </div>
      
             <div class="form-group">
              <label class="form-label" for="basic-default-name"> Style name</label>
              <input
                type="text"
                class="form-control"
                id="basic-default-name"
                name="sname" 
                placeholder="style name" value="<?php echo $sname;?>"
              />
            </div>
      <div class="form-group">
              <label class="form-label" for="basic-default-name"> Style Image</label>
              <div class="custom-file">
                <input type="file" 
                class="custom-file-input"  accept='image/*' name="profile" aria-invalid="false"  
          ...
          <img src="profiles/<?php echo $img;?>" height="60px" width="60px"  
            ...
            <br>
            <div class="form-group">
              <label class="form-label" for="basic-default-name"> Style price</label>
              <input
                type="number"
                class="form-control"
                name="price" 
                placeholder="style price" value="<?php echo $price;?>"
              />
            </div>
     ...
     <script type="text/javascript">
      ...
      $(document).ready(function() {
          $("#custom_val").validate({
                ...
                messages : {
                      sname: {
                      required: "Style name should be at least 3 characters"
                    }
                }
